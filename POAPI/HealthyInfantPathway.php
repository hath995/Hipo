...
class HealthyInfantPathway extends Pathway {
	...
	public function __construct($name,$qualifier,$session) 
	{
		...
		if($db->success())
		{
			...
			{
				$this->pathway_levels[$level['level_num']]=new TrackingDecorator(new LevelDecorator(new Level($level['level_num'],$level['name'],$this->pathway_name,$this->path_qualifier,$this->path_session)));
	...
	public function addLevel()
	{
		...
		if($db->success())
		{
			...
			if($db->success())
			{
				$this->pathway_levels[$newlevel]=new TrackingDecorator(new LevelDecorator(new Level($newlevel,"new level '.$newlevel.'",$this->pathway_name,$this->path_qualifier,$this->path_session)));
